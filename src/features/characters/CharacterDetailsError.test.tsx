...
describe('CharacterDetailsError', () => {
  beforeAll(() => {
  ...
  it('shows a not found toast and navigates back to the home page when the error is a 404', () => {
    ...
    render(<CharacterDetailsError error={error} />);
  ...
  it('shows a generic error toast for a non-axios error', () => {
    render(<CharacterDetailsError error={new Error('boom')} />);
  ...
  it('shows a generic error toast when the error is undefined', () => {
    render(<CharacterDetailsError error={undefined} />);
  ...
  it('renders nothing', () => {
    const { container } = render(<CharacterDetailsError error={undefined} />);
  ...
  it('does not show the toast more than once when re-rendered', () => {
    const error = { isAxiosError: true, response: { status: 404 } };
    const { rerender } = render(<CharacterDetailsError error={error} />);
    rerender(<CharacterDetailsError error={error} />);
